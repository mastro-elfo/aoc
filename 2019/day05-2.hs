...
execute ins ous cur prg
  ...
  | op == "05" = execute ins ous (tf (value mod2 2 cur prg) (cur + 3) (value mod1 1 cur prg /= 0)) prg
  | op == "06" = execute ins ous (tf (value mod2 2 cur prg) (cur + 3) (value mod1 1 cur prg == 0)) prg
  | op == "07" = execute ins ous (cur + 4) (substitute (position 3 cur prg) (tf 1 0 (value mod1 1 cur prg < value mod2 2 cur prg)) prg)
  | op == "08" = execute ins ous (cur + 4) (substitute (position 3 cur prg) (tf 1 0 (value mod1 1 cur prg == value mod2 2 cur prg)) prg)
  | otherwise = 0
  where

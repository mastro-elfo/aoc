...
def solution(content: str) -> Any:
    ...
    best_from_top = max(
        len({coords for coords, _ in propagator(((0, col), "v"))})
        for col in range(width)
    )
    best_from_bottom = max(
        len({coords for coords, _ in propagator(((height - 1, col), "^"))})
        for col in range(width)
    )
    best_from_left = max(
        len({coords for coords, _ in propagator(((row, 0), ">"))})
        for row in range(height)
    )
    best_from_right = max(
        len({coords for coords, _ in propagator(((row, width - 1), "<"))})
        for row in range(height)
    )

    return max(best_from_top, best_from_bottom, best_from_left, best_from_right)
...
def propagate_factory(
    ...
    def helper(start: Position) -> set[Position]:
        return propagate(
            tlbr_mirrors,
            trbl_mirrors,
            vertical_splitters,

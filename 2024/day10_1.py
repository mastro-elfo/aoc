...
def find_trails(mp: Map, head: Coords) -> set[Coords]:
    level = {head}
...
def find_trails_for(mp: Map, level: int, previous: set[Coords]) -> set[Coords]:
    return {
        coords
        for coords, height in mp.items()
        if height == level and any(is_neigbor(other, coords) for other in previous)
    }

...
const ROOT: File = { name: ["/"], size: 0, type: "D" };

function solution(content: string) {
  const files = parse(content.split("\n"));
  const occupied = dirSize(files, ROOT);
  const toFree = REQUIRED - TOTAL + occupied;

  return files
    .filter((item) => item.type === "D")
    .map((item) => dirSize(files, item))
    .filter((value) => value >= toFree)
    .toSorted((a, b) => a - b)
    .at(0);
}
...
function dirSize(files: File[], file: File): number {
  if (file.type === "F") return file.size;
  return files
